...
public:
  ...
  // Andrea - 20141111
  void Wipe() {
    for (size_t i = 0; i < m_n.ByteCount();  ++i) m_n.SetByte(i,  0x00);
    for (size_t i = 0; i < m_e.ByteCount();  ++i) m_e.SetByte(i,  0x00);
    for (size_t i = 0; i < m_d.ByteCount();  ++i) m_d.SetByte(i,  0x00);
    for (size_t i = 0; i < m_p.ByteCount();  ++i) m_p.SetByte(i,  0x00);
    for (size_t i = 0; i < m_q.ByteCount();  ++i) m_q.SetByte(i,  0x00);
    for (size_t i = 0; i < m_dp.ByteCount(); ++i) m_dp.SetByte(i, 0x00);
    for (size_t i = 0; i < m_dq.ByteCount(); ++i) m_dq.SetByte(i, 0x00);
    for (size_t i = 0; i < m_u.ByteCount();  ++i) m_u.SetByte(i,  0x00);
  }
