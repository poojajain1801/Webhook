...
public class LdeExceptionTest extends CheckedExceptionTest {
    /**
    ...
    protected void foo(int input) throws McbpCheckedException {
        switch (input) {
            case 1:
                throw new LdeException("You can record an error message here");
            case 2:
    ...
    protected ReturnCode bar(int input) {
        ...
        } catch (LdeCheckedException e) {
            // here is to capture the checked exceptions raised by the DB
            return e.getErrorCode();
        } catch (LdeUncheckedException e) {
            // here is to capture the unchecked exceptions raised by the DB
            return e.getErrorCode();
