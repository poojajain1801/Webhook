...
public class AESUtil {
    ...
    public static byte[] cipherCcm(byte[] inBuff, byte[] key, byte[] iv, boolean encrypt) throws GeneralSecurityException {
        ...
        Cipher in = Cipher.getInstance("AES/CTR/NoPadding");
        in.init(encrypt?Cipher.ENCRYPT_MODE:Cipher.DECRYPT_MODE, keyObj, new IvParameterSpec(iv));
        return in.doFinal(inBuff);
    }
    ...
    public static void main(String[] args) {
        ...
        MyJson myJson = new MyJson("var1", "var2");

        JSONSerializer serializer = new JSONSerializer();
        String serializedObj = serializer.serialize(myJson);
        ...
        //JSONSerializer
        /*RemoteManagementSessionData remoteManagementSessionData = new RemoteManagementSessionData("version",
                "sessionCode",
                "expiryTimestamp",
                "validForSeconds",
                "pendingAction",
                "tokenUniqueReference");
        byte[] buff = SerializationUtil.serializeObject(remoteManagementSessionData);
        String serializeObj = ArrayUtil.getHexString(buff);*/
    ...
    public static void encProvisionMPAReq() {
        JSONObject provisionReqData = new JSONObject();
        provisionReqData.put("requestId", "3000000001");
        provisionReqData.put("tokenUniqueReference", "DWSPMC000000000fcb2f4136b2f4136a0532d2f4136a0532");
        byte[] data = provisionReqData.toString().getBytes();
        byte[] key = ArrayUtil.getByteArray("4245BB09534AAE4DA6C5D52E5A074100");
        byte[] nonce = ArrayUtil.getByteArray("0102030405060708090A0B");
        System.out.println(provisionReqData);
        try {
            byte[] encData = AESUtil.cipherCcm(data, key, nonce, true);
            System.out.println(ArrayUtil.getHexString(encData));
