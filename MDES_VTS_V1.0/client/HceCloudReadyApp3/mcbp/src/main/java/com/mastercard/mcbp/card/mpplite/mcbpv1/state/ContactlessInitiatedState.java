...
import com.mastercard.mcbp.card.mpplite.mcbpv1.logic.contactless.ComputeCryptographicChecksum;
import com.mastercard.mcbp.card.mpplite.mcbpv1.logic.contactless.GenerateApplicationCryptogram;
...
final class ContactlessInitiatedState implements ContactlessReadySubState {
    /**
     ...
     * */
    ContactlessInitiatedState(final ContactlessContext contactlessContext) {
    ...
    public final byte[] processGenerateAc(final GenerateAcCommandApdu apdu) {
        return GenerateApplicationCryptogram.of(apdu, mContext).response();
    }
    ...
    public final byte[] processComputeCc(final ComputeCcCommandApdu apdu) {
        return ComputeCryptographicChecksum.of(apdu, mContext).response();
    }
